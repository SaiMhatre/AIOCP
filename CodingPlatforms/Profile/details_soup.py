...
from Profile.util import get_safe_nested_key
...
class UserData:
    ...
    def __codechef(self,un):
        ...
        details = {'status': 'Success', 'rating': int(rating), 'stars': stars, 'highest_rating': int(highest_rating),
                   ...
                   'contest_ratings': contest_rating_details_get(), 'fully_solved': full}
                   #, 'partially_solved': partial}

        return details

    def __codeforces(self,un):
        urls = {
            "user_info": {"url": f'https://codeforces.com/api/user.info?handles={un}'},
            "user_contests": {"url": f'https://codeforces.com/contests/with/{un}'}
        }
    ...
    def __spoj(self,un):
        url = 'https://www.spoj.com/users/{}/'.format(un)
    ...
    # DEPRECATED
    def __leetcode(self,un):
        url = 'https://leetcode.com/{}'.format(un)
    ...
    def __leetcode_v2(self, un):
        ...
        res = requests.post(url='https://leetcode.com/graphql',
                            json=payload,
                            headers={'referer': f'https://leetcode.com/{un}/'})
        res.raise_for_status()
        res = res.json()
        return __parse_response(res)

    def get_details(self, platform, un):
        if platform == 'codechef':
            return self.__codechef(self.__username)

        if platform == 'codeforces':
            return self.__codeforces(un)

        if platform == 'spoj':
            try:
                return self.__spoj(un)
            except AttributeError:
                raise UsernameError('User not Found')
